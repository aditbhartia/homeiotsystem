...
int main(void)
{
    ...
    //start udp_rx thread with necessary args
    udp_rx_args_t args;
    args.mutex = &lock;
    args.udp_port = UDP_RX_PORT;
    DEBUG("Rx thread about to initialize. \n");
    udp_rx_init(&args); 
    DEBUG("Rx thread initialized. \n");
    ...
    while(1)
    {
        //lock mutex before sampling and sending data
	    printf("main.c: About to lock mutex \n");
        mutex_lock(&lock);
        DEBUG("Main.c: Lock set \n");
	...
	//create message to be sent
        char* prefix = "abhartia/light: ";
        char data [100];
        sprintf(data, "%s%d\0", prefix, sample);

        //send message thru UDP
	    DEBUG("Sending: %s \n", data);
        udp_send(RELAY_NODE_ADDR, UDP_SEND_PORT_STR, data);

	//unlock mutex and sleep before restarting
        mutex_unlock(&lock);
        printf("main.c: Unlocked mutex \n");
        xtimer_sleep(2);
